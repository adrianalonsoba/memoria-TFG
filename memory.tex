...
This project has as main objective achieve a tool which allows create 3D dashboards to visualize and filter data from free software development inside any web browser. However, the data can be extracted from any source, just it must be in the Crossfilter's(\ref{sec:cf}) format to be understood by our tool.
...
\begin{itemize}
\item Have an object-oriented structure to facilitate future chart types thanks to the inheritance.
\item Fast filter response thanks to Crossfilter.
\item Be able to drag, zoom, and displace around the dashboard.
\item Have a dc.js similar interface, in order to facilitate the use if you know dc.js.
\item 
\end{itemize}
