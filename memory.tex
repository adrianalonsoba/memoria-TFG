...
Is have been used JavaScript for all the scripts in the project, and also, all the libraries we have included are written in JavaScript, these libraries are described in detail on the following sections.  
...
Let's take a look to the Stemkoski's 'Hello world' code:
...
The data set that we used for the firsts demos had an evolutionary time line with statistics of software development, we extracted the data from a JSON and then built two simple bars charts with two example values: 'commits' facing 'authors' along the time.

Here we have the entire HTML5 code to see the extensions we used in this first demo:
...
And here\footnote{\url{http://adrianalonsoba.github.io/threeboard/demo2/}} you can try it in your browser.
...
With this representation we can see an obvious relationship between commits and authors along the time, but thanks to the 3D renderer we can place them in different ways, for example we can form a 90 degrees angle easily: \ref{f:demo2} 
...
for (var i = 0; i < json_data.authors.length; i++) {
  //now we use the depth 'dimension' of the cube 
  ...
  var authorBar = new THREE.Mesh(geometry, material);
  authorBar.position.set(x, y, z);
  scene.add(authorBar);
  x+=1;
};
\end{lstlisting}

In addition, we also explored the possibility of joining both charts by rectangles to see if this brought us some information.
...
Explico como funciona la interactividad en three, y luego como se usa domevents, añado fotos con ejemplos y código

\subsection{Iteration 3: Filtering}
primero empiezo con el estudio de dc y pongo ejemplos con código y fotos con crossfilter
\subsection{Iteration 4: Framework (o Library?¿)}

explico la arquitectura de la librería, las diferentes funciones y como se usa las funciones encadenadas

\subsection{Iteration 5: Panel creation}
explico para que sirven, como se añaden, mueven y como se posicionan los charts con los puntos de anclaje, codigo y fotos
