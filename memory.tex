...
\begin{figure}
 ...
 \caption{Charts built with line shapes}
 \label{f:demoline}
\end{figure}

\subsection{Iteration 2: Interactivity and events}

Now that we have the basic charts, the next sprint requirement was to add interactivity to them. There is a specific method to interact with the scene using the mouse: The 'Raycasting', this method is to create an imaginary ray from the mouse to the scene in the camera direction and it returns an array with all the objects which intersects with the ray. Normally we only take the first intersected object (the first in the array) to do what we need. However, before to create the ray caster object, we need to add some event listeners as we will see. The first interactivity that we added was to change the color to the object in order to clarify which object is selected by mouse hovering, to do this we need two things, add a 'mouse move' event listener to refresh the mouse coordinates (needed by the ray caster)
