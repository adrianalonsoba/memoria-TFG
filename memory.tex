...
The data set contains 12677 objects like this with different values but the same keys. Now we must to create a single crossfilter index, a crossfilter dimension for each chart and a group for each dimension:
\begin{lstlisting}[frame=single]
//crossfilter index
...
Now we have all the needed elements to create our library, so the main requirement for this sprint is to build its architecture and implement it. However we will talk about this in the next chapter(\ref{chap:dai}) to avoid repeating information.
