...
In this chapter we will introduce the project objectives and its context, in order to clarify its 
basis before to dive into the technical details.
...
There are a lot of dashboards which have as objective facilitate abstractions to create charts

En la red podemos encontrar un muchos frameworks que tienen como objetivo proveer abstracciones que nos permitan crear nuestros propios dashboards de manera sencilla y con pocas líneas de código. Un dashboard es un interfaz de usuario que nos permite gestionar un determinado software y en nuestro caso, nos permite crear diferentes tipos de gráficos que nos muestren datos de desarrollo de software libre y que además pueden interactuar entre sí. Algunos ejemplos populares de estos frameworks pueden ser Kibana, una herramienta para visualizar y explorar los datos que se encuentran indexados en ElasticSearch, Freeboard, a internet of things data visualization oriented dashboard,or dc.js, una librería que nos permite visualizar graficos con un gran número de datos y poder filtralos en tiempo real. Sin embargo, todas estas librerías tienen algo en común, nos muestran los gráficos en 2D, lo cual tiene sus ventajas, ya que al ser en 2D no se necesita ninguna aceleración gráfica ni plugin extra, por lo que el 2D además de por su simplicidad de renderización(sin apenas impacto en el rendimiento de la computadora) ha sido el método mas común de representación de datos. No obstante, la representación en 2D también tiene sus desventajas, nunca podremos visualizar gráficos que tengan maa de dos dimensiones por razones obvias, además de limitar las posibilidades de colocación y de visualización.

 Estas desventajas son el motivo de inspiración de este proyecto, desmarcarse del resto de este tipo de librerías en 2D y crear una que muestre las gráficas en 3D, lo cuál es ya posible sin la necesidad de ningún plugin o controlador adicional gracias a webGL, que nos permite renderizar gráficos en 3D en casi cualquier navegador web.

On the web we can find lot of frameworks which objective is to provide abstraction that  allow us to create our own dashboards in an easy way with not to many code lines. A dashboard is an user interface that let us manage an specific software and in our case it allow us to create different kinds of charts that show free software developing data and also, they can interact between them. Some popular examples of this frameworks could be Kibana, a tool to visualize and explore data indexed on ElasticSearch, Freeboard, a dashboard orientated to data visualization of internet of things or dc.js a library that allows us to visualize charts with a huge amount of data and filter them in real time. However, all these libraries have something in common, they show us 2D charts, which has its advantages. Due to the fact that they are created in 2D, there is no need of graphic acceleration nor extra plugin, so that, despite the simplicity of 2D rendering(with  barely impact in hardware performance) it has been the most common method of data representation. Furthermore, 2D representation also has its disadvantages, we could never visualize charts with more than 2 dimensions for obvious reasons; and collocation and visualization possibilities are limited too.
...
El principal objetivo es conseguir una funcionalidad 
...
\section{dat.GUI}
\label{sec:dat}
A light weight graphical user interface for changing variables in JavaScript. Rather than modify variables manually in your code, use dat.GUI to modify them during execution. Here \footnote{\url{https://workshop.chromeexperiments.com/examples/gui/\#1--Basic-Usage}} we have its reference page
...
\begin{enumerate}
\item \textbf{Iteration 0}: This step mostly consists of investigation and learning.
\item \textbf{Iteration 1}: First simple but functional demos with no interactivity.
\item \textbf{Iteration 2}: Add interactivity and handle events.
...
As we can see in this example, the two frameworks have strong syntax likenesses, but finally we opted for Three.js because as we said,  it has a bigger developer community than Babylon.js.  
...
The next step before studying Three.js was to remember the Javascript language(\ref{sec:js}) and to  a Github repository to contain the project. Once that is done, the best way to learn how to use a framework is to read code and play with it (the Udacity course is very good, but it would take much time), so i preferred to follow the Stemkoski tutorials.
...
Let's have a look to the Stemkoski's 'Hello world' code housed in the appendices: \ref{sec:threehw}

In this heavily commented code we can see the fundamental aspects of a Three.js scene with no interactivity except for the mouse controls provided by the Three.js extension 'Orbit Controls' which allows us to rotate, zoom and displace the scene, you can see the final result here \ref{fig:tut} and try the demo into your web browser here\footnote{\url{http://stemkoski.github.io/Three.js/HelloWorld.html}}  
...
The first iteration objective was to explore how to render different Charts types with Three.js, the main chart types we will create are the bars chart to represent discrete values, line chart for continues values and pie charts. In order to follow an appropriate  difficulty curve and for simplicity, the first chart we created was the bars chart, because the 'BoxGeometry(or CubeGeometry)' function provides us everything we needed.
...
In addition, we also explored the possibility of joining both charts by rectangles to see if this could give us some information, however, we decided not to include this representation for the moment. You can see the result here \footnote{\url{http://adrianalonsoba.github.io/threeboard/demo5/}}
...
Now we will focus on the continuous line chart representation, this is a rather more complicated representation than the previous one because we have to draw the line manually. In this case, there is no defined mesh geometry in Three.js that could be useful, so we had to do it with the object 'THREE.Shape'. With that, we can draw any shape by connecting points with different methods,
.Once this is donde, we must give depth to the shape with 'THREE.ExtrudeGeometry', a geometry type for the handmade shapes. Here is a code example of how to make a simple line shape made with straight lines:
...
Now we have all the needed elements to create our library, so the main requirement for this sprint is to build the main architecture and implement it, after this sprint, we should be able to create the previous demo iteration with a few code lines. 

The first thing we needed is a global object that will contain the data structures, methods and it gives name to the framework, this object is \textbf{THREEDC}(three dimension 'dynamic' charting). THREEDC shall have three main methods and one data structure at least:
\begin{itemize}
...
\item \textbf{THREEDC.removeAll()} : The same as the previous , but removing all charts.
...
The code follows an object-oriented programming approximation, as we can see in \ref{fig:Architecture}, we have a global object that contain the data structures and methods, \textbf{THREEDC}
 ...
 \item \textbf{textLabel}: The single 3D text showed with data of a chart part.
 ...
 \begin{itemize}
 \item \textbf{addPanel(coords,numberOfCharts)}: This method adds a panel which will contain related charts, allowing us to handle them together, it receives coordinates to place the panel, the number of the charts that will contain, and the size.
...
\item \textbf{remove}: This method removes the chart and all its elements before removing it from THREEDC.allCharts.
...
\begin{itemize}
\item \textbf{group}:Sets the Crossfilters's group of the chart, necessary to build the charts, that is mandatory.
\item \textbf{dimension}:Sets the Crossfilters's dimension, necessary to build the charts,that is mandatory.
...
\begin{itemize}
\item \textbf{Single filter}: To apply it, we must click on some chart's part, and its dimension's value will be added to the Crossfilter's filter, when the filter is applied, the charts are rebuilt with its values filtered.
...
As we can see in \ref{fig:p1}, the chart has been placed in the first free anchor point, and if you have a look to the upper right corner of the capture, you will see this \ref{fig:gui}, a simple graphical user's interface where we can set the drag settings. The drag have to be enabled because if we want to drag a single chart, we need to click on it, but maybe we don't want to apply a filter, so, by activating the drag we disable the filter events to make sure we drag the charts easily without applying any filter. Then, when we finish the drag, we must disable it to get back the filter events.
...
\begin{figure}
 ...
 \caption{If we set the opacity property in charts and panels, we will can see through them, so we can put panels and charts behind others and compare them easily}
 \label{f:ex1}
\end{figure}
...
We can say we have fulfilled the main objectives we set at the beginning of the project, that is, achieve a similar functionality to DC.js made in 3D using THREE.js and webGL with free software. That means to build a framework that allow us to create custom 3D dashboards with filtering possibilities inside any web browser, thanks to the webGL's open graphics library.
 So, if you have any idea about how to use DC.js and THREE.js, you can create your own 3D dashboards easily with few code lines, if you don't, just take a look to the API \footnote{\url{https://github.com/adrianalonsoba/threeboard/blob/master/docs/api.md}}.
 
 However, there are additional features provided by the 3D rendering which are not possible in 2D, like overlapping charts, we can see through them thanks to the opacity setting, the 3D drag and the camera rotation features. We can also rotate the charts to place them in different angles (not implemented yet, but it will be in the near future). Of course we can place the charts and the panels at any 3D coordinates. You can see examples of this possibilities with real data sets in the previous chapter: \ref{sec:examples}.
...
And here we have the result: \ref{f:pies}

\begin{figure}
 \centering
  \subfloat[Pie rendered with dc.js]{
    \includegraphics[width=0.50\textwidth]{img/piedc.png}}
  \subfloat[Pie rendered with THREEDC.js]{
    \includegraphics[width=0.50\textwidth]{img/threedcpie.png}}
 \caption{Two pie charts with the same data.}
 \label{f:pies}
\end{figure}
...
There is a lot of subjects related to this project in my degree, at least one-third of the subjects has some type of programming, a big number if we consider that this is not a computer engineering
...
\item Use of \LaTeX by developing the project memory, an interesting  document preparation system.
\item English level improved, this is my first text written in English and although it has been difficult , I have learned a lot.
\end{itemize}

\section{Future work}
\label{sec:fw}

"A software project is never ends" said several times by several teachers along the degree, there is always something to add, something to improve or something to fix, and here we have some examples:
\begin{enumerate}
\item Graphics improvements: add shadows, textures, reflections,... Without neglecting the performance.
...
\item Drag improvements through making drag groups, that applies the same space transformation to all the meshes contained in the groups.
\item To implement charts rotation.
