...
On the web we can find lot of frameworks which objective is to provide abstraction that  allow us to create our own dashboards in an easy way with not to many code lines. A dashboard is an user interface that let us manage an specific software and in our case it allow us to create different kind of charts that show free software developing data and also, they can interact between them. Some popular examples of this frameworks could be Kibana, a tool to visualize and explore data indexed on ElasticSearch, Freeboard, a dashboard orientated to data visualization of internet of things or dc.js a library that allow us to visualize charts with a huge amount of data and filter them in real time. However, all these libraries have something in common, they show us 2D charts, which has its advantages. Due to the fact that they are created in 2D, there is no need of graphic acceleration nor extra plugin, so that despite simplicity of 2D rendering(with  barely impact in hardware performance) it has been the most common method of data representation. Furthermore, 2D representation also has its disadvantages, we could never visualize charts with more than 2 dimensions for obvious reasons; and collocation and visualization possibilities are limited too.

Those disadvantages are the inspiring reason of this project, be different of the other kind of libraries and to create one which render 3D charts, what is already possible without any plugin or additional driver thanks to webGL, that allow us to render 3D graphics in almost any web browser.
...
We can say we have fulfilled the main objectives we set at the beginning of the project, this is, achieve a similar functionality to DC.js made in 3D with THREE.js and webGL with free software. This means create a framework that allow us to create custom 3D dashboards with filters possibility inside any web browser, thanks to the webGL's open graphics library.
 So, if you have any idea of how to use DC.js and THREE.js, you can create your own 3D dashboards easily with few code lines, if you don't, just take a look to the API \footnote{\url{https://github.com/adrianalonsoba/threeboard/blob/master/docs/api.md}}.
 
 However, there is additional features provided by the 3D rendering which are not possible in 2D, like overlapping charts, we can see trough them thanks to the opacity setting, the 3D drag and the camera rotation features, also we can rotate the charts to place them in different angles (not implemented yet, but it will be in the near future). Of course we can place the charts and the panels at any 3D coordinates. You can see examples of this possibilities with real data sets in the previous chapter: \ref{sec:examples}.
 
It's important to mention that the framework is 100\% compatible with THREE.js, if you have any scene and you want to add a 3D chart to it, just need to call the  framework's code inside your scene.

To finish this section, we will create two pie charts to illustrate the similarities between DC.js and THREEDC.js syntax.
...
"A software project is never finished" said several times by several teachers along the degree, there always something to add, something to improve or something to fix, and here we have some examples:
\begin{enumerate}
