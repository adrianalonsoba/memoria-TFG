...
OJO CAMBIAR ESTO SI AL FINAL AÑADO ANIMACIONES
...
Doing this, we can move any mesh around the plane. Rotating the plane we can move the meshes in any direction, you can check it here\footnote{\url{http://adrianalonsoba.github.io/threeboard/demo15/}}.
...
AQUI MUESTRO COMO HAGO LO MISMO QUE ANTES PERO USANDO EL FRAMEWORK
...
Is have been used two remarkable techniques widely used in Javascript, a \textbf{mixin} to provide us inheritance between the different chart types avoiding the need of repeating common code and the method \textbf{chaining}, a technique that can be used to simplify code in scenarios that involve calling multiple functions on the same object consecutively.

The mixin will act like a father class, containing the common features in all charts, like render methods, events, etc... This also results in an easier abstraction. 

The basic component of a chart will be a 'part,' a single mesh that will contain info of the crossfilter's groups, this is, keys and values, so the mixin must have another data structure to save this parts. Every single chart must build each part separately because the interaction events have to be bonded 
...
\begin{itemize}
\item Implement interval filters.
...
In this chapter we will describe the library's architecture in detail, showing how it works with the basic functions and data structures. 
Let's take a look to a simplification of the library's architecture \ref{fig:Architecture}:
