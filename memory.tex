...
The data set contains 12677 objects like this with different values but the same keys. Now we must to create one crossfilter index, a crossfilter dimension for each chart and a group for each dimension:
\begin{lstlisting}[frame=single]
//crossfilter index
...
Now we can use the groups to build our charts, one chart will contain the number of commits by month and the other the number of commits by organization. In the first 
