...
EXTENDEREME MAS AQUI, HAY QUE PONER QUE EN EL CAPITULO ANTERIOR AL FINAL SE VE TODA LA FUNCIONALIDAD Y LA POTENCIA DE LA BIBLIOTECA, CON EJEMPLOS DE COMO SE HARIA CON DC Y COMO SE HACE CON ESTO, QUE SE PUEDE HACER, COMO SE PUEDE HACER Y QUE VENTAJAS TIENE, COMO POR EJEMPLO QUE TENIENDO CUALQUIER ESCENA HECHA EN TRHEE, SE PUEDE AÑADIR CUALQUIER COSA DE ESTA BIBLIOTECA FACILMENTE SIMPLEMENTE LLAMANDO AL CODIGO, ES FACIL CREAR TU PROPIOS CHARTS SIGUIENDO LA ESTRUCTURA DEL MIXIN EL MIXIN
...
We can say we have fulfilled the main objectives we set at the beginning of the project, this is, achieve a similar functionality to DC.js made in 3D with THREE.js and webGL with free software. This means create a framework that allow us to create custom 3D dashboards with filters possibility inside any web browser, thanks to the webGL's open graphics library.
 ...
 However, there is additional features provided by the 3D rendering which are not possible in 2D, like overlapping charts, we can see trough them thanks to the opacity setting, the 3D drag and the camera rotation features, also we can rotate the charts to place them in different angles (not implemented yet, but it will be in the near future). Of course we can place the charts and the panels at any 3D coordinate.
 
 It's important to mention
 
  You can see examples of this possibilities with real data sets in the previous chapter: \ref{sec:examples}.
