...
\begin{itemize}
\item \textbf{group}:Sets the Crossfilters's group of the chart, mandatory.
\item \textbf{dimension}:Sets the Crossfilters's dimension, mandatory.
...
\begin{figure}
  ...
  \caption{Empty panel example}
  \label{fig:ep}
\end{figure}

Note that the panels are interdependent to the charts and there is no need to call any render method to see them alone.

Until now, the charts are placed just using coordinates, but now we can place them into the panels, to do this, we can pass the the panel which we want to contain the charts instead of the coordinates. 

-añadir que se puede hacer drag tanto de los charts de forma individual como de un panel entero
PONER VARIOS EJEMPLOS DE CAPTURAS CON DEMOS EN DIFERENTES POSICIONES 
...
\item Graphics improve: add shadows, textures, reflections,... Without neglecting performance.
\item Add more chart types and charts with more dimensions.
\item Add some type of animations.
\item Get the data from Elasticsearch and achieve integrations with Kibana.
\item Drag improve through making drag groups, this applies the same space transformation to all the meshes contained in the groups, improving the performance.
\end{enumerate}
