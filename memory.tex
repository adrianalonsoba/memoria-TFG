...
It has been a long time, long time fighting and suffering, with many moments of despair. All these moments had led me to surrender if I had been alone. But it was never the case, thanks to my mother for encourage me, thanks to my father and my brother for serving as inspiration and thanks to my girlfriend for tell me every day: "you can do this", but above all, thanks for your love. Special mention to Jesús, one of the best tutors and teachers who you may have, and to my friends, for helping me to forget the bad moments.

To conclude i would like to say one of my favourite sentences, once told by a very wise one: 

\textit{"Querer es poder"} 

\textit{"If you want, you can"}
...
This project has as main objective to achieve a system which to allows create 3D dashboards to visualize and filter data from software development inside any web browser. However, the data can be extracted from any source, not just from software development.
...
\begin{enumerate}
\item Achieve an interface and functionality as similar as the Dc.js as possible with various chart types and filter possibilities.
\item Use any webGL framework, in order to render the dashboard inside the web browser.
\item As fast filter response as we can.
\item Be able to drag, zoom, and displace around the dashboard.
\item Be able to place independent charts at any position.
\item Be able to place the charts inside panels, to handle related charts together.
\item Have an object-oriented structure to facilitate future chart types thanks to the inheritance and abstraction.
\end{enumerate}
...
To render 3D charts, we must tell Three.js the canvas element that will contain our beautiful crafted charts:
...
\section{Iteration 0: Investigation and learning} 
...
Once we choose Three.js to write our framework, the next step was to study the foundations of 3D interactive graphics. If you are a passionate video games player (better known as 'gamer') like me, you will be familiar with concepts like rendering, game engine, fps(frames per second), image resolution, pixels, GPU(graphics processing unit)... etc. If you don't, the first lesson of the Udacity course mentioned in the Three.js's section~\ref{sec:titp}, would be helpful.

The next step before studying Three.js was to remember the Javascript language(\ref{sec:js}) and create a Github repository to contain the project. Once that is done, the best way to learn how to use a framework is to read code and play with it (the Udacity course is very good, but it would take much time), so i preferred to follow the Stemkoski's tutorials\footnote{\url{http://stemkoski.github.io/Three.js/}}.
...
\section{Iteration 1: First demos}
...
\section{Iteration 2: Interactivity and events}
...
\section{Iteration 4: Framework creation}
...
\section{Iteration 5: Panel creation and additional features}
...
We can say we have fulfilled the main objectives we set at the beginning of the project, that is, achieve a similar functionality to DC.js made in 3D using THREE.js and webGL with free software. That means to build a framework that allow us to create custom 3D dashboards with filtering possibilities inside any web browser, thanks to the webGL's open graphics library.
 So, if you have any idea about how to use DC.js and THREE.js, you can create your own 3D dashboards easily with few lines of code, if you don't, just take a look to the API \footnote{\url{https://github.com/adrianalonsoba/threeboard/blob/master/docs/api.md}}.
...
\section{Three.js and Babylon.js comparison}
