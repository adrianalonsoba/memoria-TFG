...
As we can see in this example, the two frameworks have strong syntax likeness, but  
Once we choose Three.js to write our framework, the next step was to brush up the javascript language and start to learn the basis of 3D computer graphics(PARA ESTO SON BUENOS LOS PRIMEROS PASOS DEL CURSO DE UDACITY) and Three.js(PONGO LOS EJEMPLOS  Y LOS COMENTO CON CODIGO Y FOTOS ), 

\subsection{First demo}

\subsection{Interactivity and events}
\subsection{Filtering}
primero empiezo con el estudio de dc y pongo ejemplos con código y fotos, luego sigo con lo mas tocho de todo
