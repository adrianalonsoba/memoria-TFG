...
All we have to do is to take some data set and assign each value to the height parameter of the BoxGeometry function, the width value and the depth value are not much important for now. 
...
Once we created the scene in 'script.js' as we learned, we add the custom bars to it:
...
\end{figure}
 ...
 PONGO  UN EJEMPLO COMPLETO CON EL HTML PARA QUE SE VEAN TODOS LOS SCRIPTS, EXPLICO PARA QUE VALA CADA UNO, LUEGO EL CODIGO(SOLO EL CUSTOM) Y EXPLICO COMO LO PINTO TAMBIEN CON EL JSON, PONGO CAPTURAS Y CÓDIGO DE  LAS DISTINTAS VISUALIZACIONES ENTRE LAS DEMOS 1-5 MAS O MENOS, OBVIANDO LA INTERACTIVIDAD
