...
\item Be able to fix some chart parts by clicking.
...
Now that we have all the tools to handle events let's go to add a 3D text label to each mesh showing the number of commits or authors. To do this, we can use 'THREE.TextGeometry', a function that makes simple to add 3D text to the scenes just by passing the text, the font family, the size and other optional parameters. 
...
The second iteration objective is not included in the final version but it gave us an idea of how fast the renderer can add and remove simple meshes from the scene. In this\footnote{\url{http://adrianalonsoba.github.io/threeboard/demo6/}}demo you can try it by doing mouse over and click to fix bars.

The third requirement is probably the most important of the iteration. LO PONGO AQUI O NO¿? TENGO QUE EXPLICAR COMO USO A LA VEZ EL DOM EVENTS Y EL RAYCASTING AQUI
...
explico la arquitectura de la librería, las diferentes funciones y como se usa las funciones encadenadas muy resumido

\subsection{Iteration 5: Panel creation}
explico para que sirven, como se añaden, mueven y como se posicionan los charts con los puntos de anclaje,
